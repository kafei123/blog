...
if (dateArray[i] != "&nbsp;"){ 
tds[i].bgColorTxt = "td_bg_out"; //保存背景色的 class 
var cur = new Date(); 
tds[i].isToday = false; 
...
var now = new Date(); 
this.date = (dateObj.value.length > 0) ? new Date(dateObj.value.toDate(this.dateFormatStyle)) : now.format(this.dateFormatStyle).toDate(this.dateFormatStyle) ;//添加 → 若为空则根据dateFormatStyle初始化日期 
...
//以下 → 确保日历容器节点在 body 最后，否则 FireFox 中不能出现在最上方 
